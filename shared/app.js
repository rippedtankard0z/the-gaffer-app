'use strict';
        ...
        const Fixtures = ({ categories, opponents, venues, referees, refDefaults, seasonCategories, setOpponents, setVenues, onNavigate, launchMode = 'schedule' }) => {
            ...
            const matchdayUpcomingFixtures = useMemo(() => {
                if (!fixtures.length) return [];
                ...
                return fixtures
                    ...
                    .map(fixture => {
                        const ts = toTs(fixture.date);
                        return {
                            fixture,
                            ts: ts === null ? Number.MAX_SAFE_INTEGER : ts
                        };
                    })
                    .filter(row => row.ts >= todayTs || row.ts === Number.MAX_SAFE_INTEGER)
                    .sort((a, b) => a.ts - b.ts)
                    .map(row => row.fixture);
            }, [fixtures]);
            useEffect(() => {
            ...
            const pickMatchdayFixture = useCallback((list = []) => {
                ...
                const todayFixtures = list
                    ...
                    .sort((a, b) => (toTs(a.date) || 0) - (toTs(b.date) || 0));
            ...
            const isMatchdayWorkspace = launchMode === 'matchday';
        ...
        const App = () => {
            ...
            const appBuildInfo = useMemo(() => {
                return READ_ONLY ? formatBuildLabel(APP_VERSION, true) : formatBuildLabel(APP_VERSION, false);
            }, []);
